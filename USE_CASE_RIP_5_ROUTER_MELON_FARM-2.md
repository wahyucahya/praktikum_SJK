...
## 🗺️ TOPOLOGI JARINGAN 5 ROUTER (dengan Detail Interface)
...
```
    ...
    |     Gi0/2: WAN (203.0.113.x dari ISP)      |
    |     Gi0/1: DMZ (192.168.5.1/24)            |
    |     Gi0/0: to R4 (10.0.5.2/30)             |
    +---------------------------------------------+
              Gi0/0 |
                    | 10.0.5.0/30 Link
              Gi0/3 |
    ...
    |     Gi0/0: to R1 (10.0.1.2/30)             |
    |     Gi0/1: to R2 (10.0.2.2/30)             |
    |     Gi0/2: to R3 (10.0.3.2/30)             |
    |     Gi0/3: to R5 (10.0.5.1/30)             |
    |     Fa0/0: LAN Office (192.168.4.1/24)     |
    +---------------------------------------------+
         |           |           |
    Gi0/0|      Gi0/1|      Gi0/2|
         |           |           |
...
## 🔌 MAPPING INTERFACE DETAIL - CONNECTION DIAGRAM
...
│         ROUTER 1 - ZONA UTARA               │
│                                             │
│  [Gi0/0] ────► Switch 1 (LAN Zona 1)       │
│   IP: 192.168.1.1/24                        │
│   Connect: Sensors & Actuators              │
│                                             │
│  [Gi0/1] ────► Router 4 (Gi0/0)            │
│   IP: 10.0.1.1/30                           │
│   Cable: Fiber Optic SM (~250m)             │
│                                             │
│  [Gi0/2] ────► Not Used                    │
│  [Gi0/3] ────► Not Used                    │
└─────────────────────────────────────────────┘
         ...
         ├──► [Switch 1]
         │      │
         │      ├──► Sensor Suhu-1 (192.168.1.10)
         │      ├──► Sensor Tanah-1 (192.168.1.11)
         │      ├──► Sensor Humid-1 (192.168.1.12)
         │      ├──► Pompa Air-1 (192.168.1.20)
         │      ├──► Katup-1 (192.168.1.21)
         │      └──► Katup-2 (192.168.1.22)
```

### **📍 ROUTER 2 (Field Center - Zona Tengah)**
```
┌─────────────────────────────────────────────┐
│        ROUTER 2 - ZONA TENGAH               │
│                                             │
│  [Gi0/0] ────► Switch 2 (LAN Zona 2)       │
│   IP: 192.168.2.1/24                        │
│   Connect: Sensors & Actuators              │
│                                             │
│  [Gi0/1] ────► Router 4 (Gi0/1)            │
│   IP: 10.0.2.1/30                           │
│   Cable: Fiber Optic SM (~200m)             │
│                                             │
│  [Gi0/2] ────► Not Used                    │
│  [Gi0/3] ────► Not Used                    │
└─────────────────────────────────────────────┘
         ...
         ├──► [Switch 2]
         │      │
         │      ├──► Sensor Tanah-2 (192.168.2.10)
         │      ├──► Sensor Cuaca (192.168.2.11)
         │      ├──► Pompa Air-2 (192.168.2.20)
         │      ├──► Katup-3 (192.168.2.21)
         │      └──► Katup-4 (192.168.2.22)
```

### **📍 ROUTER 3 (Field South - Zona Selatan)**
```
┌─────────────────────────────────────────────┐
│       ROUTER 3 - ZONA SELATAN               │
│                                             │
│  [Gi0/0] ────► Switch 3 (LAN Zona 3)       │
│   IP: 192.168.3.1/24                        │
│   Connect: Sensors & Actuators              │
│                                             │
│  [Gi0/1] ────► Router 4 (Gi0/2)            │
│   IP: 10.0.3.1/30                           │
│   Cable: Fiber Optic SM (~300m)             │
│                                             │
│  [Gi0/2] ────► Not Used                    │
│  [Gi0/3] ────► Not Used                    │
└─────────────────────────────────────────────┘
         ...
         ├──► [Switch 3]
         │      │
         │      ├──► Sensor Suhu-2 (192.168.3.10)
         │      ├──► Sensor Tanah-3 (192.168.3.11)
         │      ├──► Sensor Humid-2 (192.168.3.12)
         │      ├──► Pompa Air-3 (192.168.3.20)
         │      └──► Katup-5 (192.168.3.21)
```
...
│    ROUTER 4 - OFFICE CONTROL CENTER (HUB)   │
│                                             │
│  [Gi0/0] ────► Router 1 (Gi0/1)            │
│   IP: 10.0.1.2/30                           │
│   Cable: Fiber Optic SM (~250m)             │
│                                             │
│  [Gi0/1] ────► Router 2 (Gi0/1)            │
│   IP: 10.0.2.2/30                           │
│   Cable: Fiber Optic SM (~200m)             │
│                                             │
│  [Gi0/2] ────► Router 3 (Gi0/1)            │
│   IP: 10.0.3.2/30                           │
│   Cable: Fiber Optic SM (~300m)             │
│                                             │
│  [Gi0/3] ────► Router 5 (Gi0/0)            │
│   IP: 10.0.5.1/30                           │
│   Cable: Fiber Optic SM/MM (~120m)          │
│                                             │
│  [Fa0/0] ────► Switch 4 (LAN Office) ✅     │
│   IP: 192.168.4.1/24                        │
│   Cable: UTP Cat6 (10m)                     │
│   Connect: Server, Admin PC, etc.           │
└─────────────────────────────────────────────┘
         │ Fa0/0
         ├──► [Switch 4 - Office]
         │      │
         │      ├──► Server Database (192.168.4.10)
         │      ├──► Admin PC (192.168.4.20)
         │      ├──► Printer (192.168.4.30)
         │      ├──► Laptop Manager (192.168.4.40)
         │      ├──► NVR CCTV (192.168.4.50)
         │      ├──► WiFi AP (192.168.4.60)
         │      └──► Monitor Dashboard (192.168.4.100)
```
...
│    ROUTER 5 - EDGE/INTERNET GATEWAY         │
│                                             │
│  [Gi0/0] ────► Router 4 (Gi0/3)            │
│   IP: 10.0.5.2/30                           │
│   Cable: Fiber Optic SM/MM (~120m)          │
│                                             │
│  [Gi0/1] ────► Switch 5 (DMZ LAN)          │
│   IP: 192.168.5.1/24                        │
│   Cable: UTP Cat6 (5m)                      │
│   Connect: Firewall, VPN, DNS, Proxy        │
│                                             │
│  [Gi0/2] ────► ISP Modem/ONT               │
│   IP: DHCP dari ISP (203.0.113.x)           │
│   Cable: Fiber dari ISP (~60m)              │
│   ⚠️ WAN Interface - Internet Access         │
│                                             │
│  [Gi0/3] ────► Not Used                    │
└─────────────────────────────────────────────┘
         │ Gi0/1
         ├──► [Switch 5 - DMZ]
         │      │
         │      ├──► Firewall Server (192.168.5.10)
         │      ├──► VPN Server (192.168.5.20)
         │      ├──► DNS Cache (192.168.5.30)
         │      ├──► Proxy Server (192.168.5.40)
         │      └──► Cloud Sync (192.168.5.50)
         │
         │ Gi0/2 (WAN)
         └──► [ISP ONT/Modem] ──► ☁️ INTERNET
```

---

## 📝 CATATAN PENTING - SOLUSI Gi0/4

### **⚠️ Masalah: Router Standard Hanya Punya Gi0/0 sampai Gi0/3**

Karena **Router 4 membutuhkan 5 interface** (3 ke field + 1 ke edge + 1 ke LAN), ada 3 solusi:

#### **✅ SOLUSI 1: Gunakan FastEthernet Fa0/0 untuk LAN Office (RECOMMENDED)**

```
Router 4 Interface Assignment:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
┃ Interface ┃ Connected To     ┃ Type   ┃ Speed  ┃
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
┃ Gi0/0     ┃ Router 1 (Field) ┃ Uplink ┃ 1 Gbps ┃
┃ Gi0/1     ┃ Router 2 (Field) ┃ Uplink ┃ 1 Gbps ┃
┃ Gi0/2     ┃ Router 3 (Field) ┃ Uplink ┃ 1 Gbps ┃
┃ Gi0/3     ┃ Router 5 (Edge)  ┃ Uplink ┃ 1 Gbps ┃
┃ Fa0/0     ┃ Switch 4 (Office)┃ LAN    ┃100Mbps ┃✅
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```

**Keuntungan:**
- ✅ Tidak perlu expansion module
- ✅ FastEthernet 100Mbps cukup untuk LAN office (7 devices)
- ✅ Lebih murah & simple
- ✅ Support di semua Cisco Router (2900/2800 series)
...
#### **SOLUSI 2: Tambah Expansion Module HWIC/NM**

Jika ingin semua interface Gigabit, beli module:

| Module | Ports | Speed | Harga | Kegunaan |
|--------|-------|-------|-------|----------|
| HWIC-4ESW | 4 ports | 10/100 | ~Rp 2M | Switch module |
| HWIC-2FE | 2 ports | 10/100 | ~Rp 1.8M | 2x FastEthernet |
| NM-1FE-TX | 1 port | 10/100 | ~Rp 1.5M | 1x FastEthernet |
| HWIC-1GE-SFP | 1 port | 1 Gbps | ~Rp 3M | 1x Gigabit SFP |

**Instalasi di Cisco Packet Tracer:**
1. Power off router
2. Drag module ke slot HWIC atau NM
3. Power on
4. Verify: `show ip interface brief`

**New Interface:**
```
FastEthernet1/0 (dari HWIC-2FE)
atau
GigabitEthernet1/0 (dari HWIC-1GE-SFP)
```

---

#### **SOLUSI 3: Gunakan Layer 3 Switch (Advanced)**

Replace Router 4 dengan **Cisco Catalyst 3560** atau **3750**:
- Support inter-VLAN routing
- Banyak port (24/48 port)
- Support RIP/OSPF

**Konfigurasi L3 Switch:**
```cisco
ip routing
router rip
 version 2
...
## 🔗 TABEL KONEKSI LENGKAP ANTAR KOMPONEN

### **Router ke Router Links:**

| Dari Router | Interface | IP Address | Ke Router | Interface | IP Address | Kabel Type | Jarak |
|-------------|-----------|------------|-----------|-----------|------------|------------|-------|
| **R1 Field North** | Gi0/1 | 10.0.1.1/30 | **R4 Office** | Gi0/0 | 10.0.1.2/30 | Fiber SM | 250m |
| **R2 Field Center** | Gi0/1 | 10.0.2.1/30 | **R4 Office** | Gi0/1 | 10.0.2.2/30 | Fiber SM | 200m |
| **R3 Field South** | Gi0/1 | 10.0.3.1/30 | **R4 Office** | Gi0/2 | 10.0.3.2/30 | Fiber SM | 300m |
| **R4 Office** | Gi0/3 | 10.0.5.1/30 | **R5 Edge** | Gi0/0 | 10.0.5.2/30 | Fiber SM | 120m |
| **R5 Edge** | Gi0/2 | DHCP | **ISP** | ONT LAN | - | Fiber ISP | 60m |

### **Router ke Switch Links:**

| Router | Interface | IP Address | Switch | Port | Kabel | Jarak |
|--------|-----------|------------|--------|------|-------|-------|
| **R1** | Gi0/0 | 192.168.1.1/24 | Switch 1 | 1 | UTP Cat6 | 5m |
| **R2** | Gi0/0 | 192.168.2.1/24 | Switch 2 | 1 | UTP Cat6 | 5m |
| **R3** | Gi0/0 | 192.168.3.1/24 | Switch 3 | 1 | UTP Cat6 | 5m |
| **R4** | **Fa0/0** ✅ | 192.168.4.1/24 | Switch 4 | 1 | UTP Cat6 | 10m |
| **R5** | Gi0/1 | 192.168.5.1/24 | Switch 5 | 1 | UTP Cat6 | 5m |

### **Switch ke End Devices (Zona 1):**

| Switch | Port | Device | IP Address | Kabel |
|--------|------|--------|------------|-------|
| **Switch 1** | 2 | Sensor Suhu-1 | 192.168.1.10 | UTP Cat5e |
| **Switch 1** | 3 | Sensor Tanah-1 | 192.168.1.11 | UTP Cat5e |
| **Switch 1** | 4 | Sensor Humid-1 | 192.168.1.12 | UTP Cat5e |
| **Switch 1** | 5 | Pompa Air-1 | 192.168.1.20 | UTP Cat5e |
| **Switch 1** | 6 | Katup-1 | 192.168.1.21 | UTP Cat5e |
| **Switch 1** | 7 | Katup-2 | 192.168.1.22 | UTP Cat5e |

### **Switch ke End Devices (Zona 2):**

| Switch | Port | Device | IP Address | Kabel |
|--------|------|--------|------------|-------|
| **Switch 2** | 2 | Sensor Tanah-2 | 192.168.2.10 | UTP Cat5e |
| **Switch 2** | 3 | Sensor Cuaca | 192.168.2.11 | UTP Cat5e |
| **Switch 2** | 4 | Pompa Air-2 | 192.168.2.20 | UTP Cat5e |
| **Switch 2** | 5 | Katup-3 | 192.168.2.21 | UTP Cat5e |
| **Switch 2** | 6 | Katup-4 | 192.168.2.22 | UTP Cat5e |

### **Switch ke End Devices (Zona 3):**

| Switch | Port | Device | IP Address | Kabel |
|--------|------|--------|------------|-------|
| **Switch 3** | 2 | Sensor Suhu-2 | 192.168.3.10 | UTP Cat5e |
| **Switch 3** | 3 | Sensor Tanah-3 | 192.168.3.11 | UTP Cat5e |
| **Switch 3** | 4 | Sensor Humid-2 | 192.168.3.12 | UTP Cat5e |
| **Switch 3** | 5 | Pompa Air-3 | 192.168.3.20 | UTP Cat5e |
| **Switch 3** | 6 | Katup-5 | 192.168.3.21 | UTP Cat5e |

### **Switch ke End Devices (Office):**

| Switch | Port | Device | IP Address | Kabel |
|--------|------|--------|------------|-------|
| **Switch 4** | 2 | Server Database | 192.168.4.10 | UTP Cat6 |
| **Switch 4** | 3 | Admin PC | 192.168.4.20 | UTP Cat6 |
| **Switch 4** | 4 | Printer | 192.168.4.30 | UTP Cat6 |
| **Switch 4** | 5 | Laptop Manager | 192.168.4.40 | UTP Cat6 |
| **Switch 4** | 6 | NVR CCTV | 192.168.4.50 | UTP Cat6 |
| **Switch 4** | 7 | WiFi Access Point | 192.168.4.60 | UTP Cat6 |
| **Switch 4** | 8 | Monitor Dashboard | 192.168.4.100 | UTP Cat6 |

### **Switch ke End Devices (DMZ):**

| Switch | Port | Device | IP Address | Kabel |
|--------|------|--------|------------|-------|
| **Switch 5** | 2 | Firewall Server | 192.168.5.10 | UTP Cat6 |
| **Switch 5** | 3 | VPN Server | 192.168.5.20 | UTP Cat6 |
| **Switch 5** | 4 | DNS Cache Server | 192.168.5.30 | UTP Cat6 |
| **Switch 5** | 5 | Proxy Server | 192.168.5.40 | UTP Cat6 |
| **Switch 5** | 6 | Cloud Sync Server | 192.168.5.50 | UTP Cat6 |

---

## 🛠️ IMPLEMENTASI DI CISCO PACKET TRACER

### **Langkah Setup Hardware:**

#### **1. Pilih Router Type yang Sesuai:**

| Location | Router Model | Reason | Interface Available |
|----------|--------------|--------|---------------------|
| Field (R1-R3) | Cisco 2911 | 4x Gi + 2x Fa | Gi0/0-3, Fa0/0-1 ✅ |
| Office (R4) | Cisco 2911 | 4x Gi + 2x Fa | Gi0/0-3, Fa0/0-1 ✅ |
| Edge (R5) | Cisco 2911 | 4x Gi + 2x Fa | Gi0/0-3, Fa0/0-1 ✅ |

**Alternative:** Cisco 2901 (jika 2911 tidak tersedia)

#### **2. Cek Interface di Packet Tracer:**

Setelah drag router ke workspace:
```
Router> enable
Router# show ip interface brief
```

**Expected Output (Cisco 2911):**
```
...
**✅ Perfect! Ada FastEthernet0/0 untuk LAN!**

#### **3. Connection Guide di Packet Tracer:**

**Kabel yang Digunakan:**
- **Router ↔ Router:** Fiber Optic (pilih "Fiber" di Packet Tracer)
- **Router ↔ Switch:** Straight-Through (Copper Straight-Through)
- **Switch ↔ PC/Device:** Straight-Through

**Cara Connect:**
1. Klik kabel (pilih jenis yang sesuai)
2. Klik source device → pilih interface
3. Klik destination device → pilih interface
4. Tunggu hingga interface berubah hijau (after config)
...
| **R4** | **Gi0/3** | **10.0.5.1/30** | **Link R4-R5** | **Point-to-Point to Edge Router** ✅ |
| **R4** | **Fa0/0** | **192.168.4.1/24** | **Office LAN** | **Control Center & Monitoring** ✅ |
...
! Konfigurasi Interface LAN Office (menggunakan FastEthernet)
